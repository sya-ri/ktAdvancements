...
package dev.s7a.ktAdvancements.runtime.v26_3
...
class KtAdvancementRuntimeImpl : KtAdvancementRuntime {
    override fun sendPacket(
        ...
        (player as CraftPlayer).handle.connection.send(
            ClientboundUpdateAdvancementsPacket(
                reset,
                advancements.keys.map {
                    PositionedAdvancement(AdvancementHolder(it.id.location(), it.convert()), it.display.x, it.display.y)
                },
    ...
    private fun NamespacedKey.location() = Identifier.fromNamespaceAndPath(namespace, key)
